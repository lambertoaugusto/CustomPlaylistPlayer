$(document).ready(function() {
    ...
    function getPlaylistData(playlistID, video_list, page_token) { //Makes a single request to Youtube Data API
        var apiKey = 'AIzaSyCYh1o4GEw1V0Sap-j_QIhng2KNNJfN1Ss';
        var theUrl =
    ...
    var showMenu = $('#mobile-indicator').is(':visible');
    ...
    window.onYouTubeIframeAPIReady = function() { // Creates an <iframe> (and YouTube player) after the API code downloads. must be globally available
        ...
        window.addEventListener('resize', function(event) {
            ...
            showMenuBefore = showMenu;
            showMenu = $('#mobile-indicator').is(':visible');
            if(showMenu != showMenuBefore){
                player.destroy();
                loadPlayer(showMenu);
            }                
        });

        $(document).on('closed.zf.reveal', '#modalPlayer', function(e) {
            player.stopVideo();
        });
    ...
    $(document).on('open.zf.reveal', '#modalPlayer', function(e) {
        onPlayerReady(e);
    });
